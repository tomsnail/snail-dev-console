...
<body>
	...
	<script type="text/template" id="treeTableTpl">
		<tr id="{{row.id}}" pId="{{pid}}">
			<td><a href="${ctx}/indexpage/jsIndex/form?id={{row.id}}">
				{{row.name}}
			</a></td>
			<td>
				{{row.orderInt}}
			...
			<shiro:hasPermission name="indexpage:jsIndex:edit"><td>
   				<a href="${ctx}/indexpage/jsIndex/form?id={{row.id}}">修改</a>
				<a href="${ctx}/indexpage/jsIndex/delete?id={{row.id}}" onclick="return confirmx('确认要删除该首页配置及所有子首页配置吗？', this.href)">删除</a>
				<a href="${ctx}/indexpage/jsIndex/form?parent.id={{row.id}}">添加下级首页配置</a> 
